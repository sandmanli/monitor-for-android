...
TZs=`$bb awk -v csv="$monitor/thermal.csv" '{split(FILENAME,N,"/");printf N[6]" "$0" ";if(R=="")R="uptime,"substr(N[6],13)":"$0;else R=R","substr(N[6],13)":"$0}END{print R>csv}' /sys/devices/virtual/thermal/thermal_zone*/type`
...
while true;do
	...
	#thermal_zone
	$bb awk -v time=$uptime -v TZs="$TZs" -v csv="$monitor/thermal.csv" 'BEGIN{split(TZs,tmp," ")}{split(FILENAME,N,"/");r[N[6]]=$0}END{for(i in tmp){if(i%2==0){if(R=="")R=r[tmp[i-1]]+0;else R=R","r[tmp[i-1]]+0}};print time","R >>csv}'  /sys/devices/virtual/thermal/thermal_zone*/temp
	#windows.csv
