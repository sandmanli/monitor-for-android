...
"""%tmp[1]
	...
	def cpus(self, csvPath):
		...
		for i in range(1, cpus+2) :
			tmp = data[data.columns[i]].astype('Float64').values.tolist()
			series.append(tmp)
			statistics.append([round(np.mean(tmp), 1), round(np.median(tmp), 1)])
		log('cpus Finish')
	...
	def cpuinfo(self, csvPath):
		...
		for c in Command:
			...
			CPU_max = [max(CPU), round(np.mean(CPU), 1), round(np.median(CPU), 1)]
	...
	def mem(self, csvPath):
		...
		if len(data.columns) == 15:
			CMAFree = (data['CMA Free']/1024).astype('Float64').values.tolist()
		Free = ((data['MemFree']+data['Buffers']+data['Cached'])/1024).astype('Float64').values.tolist()
	...
	def mem2(self, csvPath):
		...
		for j in range(1,len(name)+1):
			if name[j-1].find('_mmap') < 0 :
				...
				D_mmap_other.append((data[data.columns[j]]/1024).astype('Float64').values.tolist())
			else:
				N_mmap.append(name[j-1])
				D_mmap.append((data[data.columns[j]]/1024).astype('Float64').values.tolist())
	...
	def meminfo(self, csvPath):
		...
		for c in Command:
			...
			if len(tmp) > 0:
				Pss_max = max(tmp)
				Pss_min = min(tmp)
				Pss_mean = round(np.mean(tmp)*1.0,2)
				Pss_median = round(np.median(tmp)*1.0,2)
			else:
				Pss_max = 0
				Pss_min = 0
				Pss_mean = 0
			...
			maxpd.append(['%s'%c, h, round((Pss_max-Pss_min)*1.0,2), round(Pss_max*1.0,2), Pss_mean, Pss_median])
			h += 1
	...
	def meminfo2(self, csvPath):
		...
		for c in Command:
			...
			if len(tmp) > 0:
				VSZ_max = max(tmp)
				VSZ_min = min(tmp)
				VSZ_mean = round(np.mean(tmp)*1.0,2)
				VSZ_median = round(np.median(tmp)*1.0,2)
			else:
				VSZ_max = 0
				VSZ_min = 0
				VSZ_mean = 0
			...
			maxpd.append(['%s'%c, h, round(VSZ_max-VSZ_min,3), round(VSZ_max,3), VSZ_mean, VSZ_median])
			h += 1
		maxpd.sort(key = lambda a_tuple:a_tuple[2],reverse = True)
		log('meminfo2 Finish')
		return [maxpd, meminfo_data]
